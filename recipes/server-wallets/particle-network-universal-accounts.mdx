---
title: "Particle Network Universal Accounts"
description: "Enable user wallets via Magic's Server Wallet and cross-chain interactions with Universal Accounts"
...
This guide demonstrates how to combine **Magic’s Server Wallet** with **Particle Network’s Universal Accounts** to provide users with a secure, passwordless, and chain-agnostic Web3 experience.

With Magic's Server Wallet, developers can generate wallets through any JWT-based auth provider, including familiar **OAuth logins** (such as Google)  — no seed phrases or extensions required.
...
<Note>
  This guide is based on a demo app where users log in using **Google OAuth** (via Magic's Server Wallet) and instantly interact with **Universal Accounts** to mint NFTs across chains — without bridges or network switching.  
...
By combining **Magic’s Server Wallet** and **Universal Accounts**, developers solve both challenges:

- **Magic’s Server Wallet** generates secure, non-custodial wallets using JWT-based authentication, including simple OAuth logins like Google.
...
The integration connects **Magic’s Server Wallet** (for secure wallet creation) with **Particle Network’s Universal Accounts** (for chain abstraction).  
Here’s how the flow works from login to cross-chain interaction:

<Steps>
  <Step title="User Logs In with OAuth">
...
```
  ...
  <Step title="Initialize the Universal Account">
  ...
  <Step title="Interact Across Chains">
...
By integrating **Magic’s Server Wallet** with **Universal Accounts**, developers can provide a modern onboarding experience and eliminate multi-chain friction.
...
<Steps>
  ...
  <Step title="Set Up Environment Variables">
    ...
    Get your Magic Secret Key from the [Magic Dashboard](https://dashboard.magic.link/) and your Particle credentials from the [Particle Dashboard](https://dashboard.particle.network/).
    
    Follow the instructions [here](/server-wallets/express-api/identity-provider) to configure your identity provider.
  </Step>
...
## Conclusion
With **Magic’s Server Wallet** with **Particle Network’s Universal Accounts**, developers can onboard users with familiar OAuth flows while giving them access to a single account that works across all major blockchains — unified balance, cross-chain liquidity, and gas abstraction included.  
