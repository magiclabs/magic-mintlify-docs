...
Magic link SDK methods are available via the [Web client-side SDK](/embedded-wallets/sdk/client-side/javascript). These methods are deprecated on all mobile SDKs.

## Usage

The `loginWithMagicLink` function sends an authentication request to the user. The resolved value is a Decentralized ID token with a default 15-minute lifespan. **Refer to the **[**API documentation**](/embedded-wallets/sdk/overview) **for information on how to install and initialize Magic.**
...
* To assert a case-specific failed state, provide an [RPC error code](/embedded-wallets/sdk/client-side/javascript#rpcerror) with `test+fail_with_{RPC_ERROR_CODE}@magic.link`
...
[How to add OTP email login on iOS](/embedded-wallets/sdk/client-side/ios#loginwithemailotp)

[How to add OTP email login on Android](/embedded-wallets/sdk/client-side/android#loginwithemailotp)

[How to add OTP email login on Flutter](/embedded-wallets/sdk/client-side/flutter#loginwithemailotp)

[How to add OTP email login in React Native](/embedded-wallets/sdk/client-side/javascript#loginwithemailotp)

## Resources

- [Error Codes](/embedded-wallets/sdk/client-side/javascript#error-codes)
