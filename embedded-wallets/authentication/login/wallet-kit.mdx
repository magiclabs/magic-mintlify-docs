---
title: Wallet Kit
description: "With Magic, you can use a pre-built React component that provides a complete authentication UI with support for email, OAuth, and external wallet login."
---
...
**Code-configured options** — External wallets (MetaMask, Coinbase, etc.) are configured via the `wallets` prop. Only the wallets you specify will be displayed.
...
```jsx JavaScript icon="square-js"
const magic = new Magic('YOUR_API_KEY', {
  extensions: [new MagicWidgetExtension({ projectId: 'YOUR_REOWN_PROJECT_ID' })],
});
```
