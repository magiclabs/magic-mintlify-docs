...
If your users are encountering an "Access blocked: magic.link has not completed the Google verification process" error, please [read below](/embedded-wallets/authentication/login/oauth/social-providers/google#verification-process).
</Note>
...
<Warning>
OAuth v1 extension is deprecated. Only use for legacy implementations requiring backwards compatibility.
</Warning>
...
7.  Input the **Client ID** and **Client Secret** for your OAuth app
    ...
    a. **If using the connectWithUI SDK method:** Choose "Magic Login Widget" in your Magic Dashboard's Google Oauth settings and copy the **Redirect URI** field from your Magic Dashboard. Add this Redirect URI to your Google Dashboard's OAuth app configuration:
    
    <Accordion title="View connectWithUI SDK method Redirect URI field">
      <Frame>
...
<Tip>
Use OAuth v2 extension for all new projects.
</Tip>

* Follow above steps, but disregard Magic's **Redirect URI** in step 7.a
