...
Passkey MFA is supported in the OAuth flow. After the user returns from the OAuth redirect, Magic handles passkey verification automatically when using the default UI. For custom MFA handling, see [Passkey MFA](/embedded-wallets/authentication/features/passkey-mfa#oauth).
