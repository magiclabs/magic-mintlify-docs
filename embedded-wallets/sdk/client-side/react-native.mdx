...
<CodeGroup>
  ...
  npm install react-native-device-info # Required Peer Dependency
  npm install @react-native-community/async-storage # Required Peer Dependency
  npm install react-native-safe-area-context # Required Peer Dependency
  ...
  npm install react-native-webview@^11.26.0 # Required Peer Dependency
  npm install react-native-safe-area-context # Required Peer Dependency
...
* `showUI?` (Boolean): If `true`, show Magic's out-of-the-box UI for OTP entry and error states. Defaults to `true`. Set to `false` for a fully white-labeled flow, in which errors are rejected back to you to handle.
...
try {
  await magic.auth.loginWithSMS({ phoneNumber: "+14151231234", showUI: false });
} catch (err) {
  if (err instanceof RPCError) {
    switch (err.code) {
      case RPCErrorCode.InvalidRecaptcha:
        // reCAPTCHA verification failed. Safe to retry.
        break;
      case RPCErrorCode.InvalidParams:
        // Invalid phone number.
        break;
      default:
        // Other failures (e.g. SMS not enabled for the client, misconfiguration).
        // Inspect err.rawMessage for the server-provided reason.
        break;
    }
  }
}
...
- `PromiEvent<object>`:
  ...
  - `wallets` (Object): Organized wallet addresses by blockchain and network:
      - `[chainName]` (Object): Each supported blockchain (e.g., `ethereum`, `solana`, `bitcoin`, `cosmos`, `hedera`)
...
console.log(bitcoinAddress); // => "tb1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh"
```
