...
| `options.testMode?`        | Boolean          | Enable [`testMode`](/embedded-wallets/authentication/login/magic-links) to assert the desired behavior through the email address you provide to `loginWithMagicLink` without having to go through the auth flow.                                                                                                                                                                                                                                         |
...
* `PromiEvent<object>`:
    ...
    * `wallets` (Object):  Organized wallet addresses by blockchain and network:
        * `[chainName]` (Object): Each supported blockchain (e.g., `eth`, `solana`, `bitcoin`, `hedera`)
