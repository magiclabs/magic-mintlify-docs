...
Magic offers Wallet Pregeneration, a feature that allows you to create non-custodial EVM wallets programmatically without requiring end users to start or complete an authentication flow. After the wallet is generated, a user can claim their pregenerated wallet by authenticating through your application.
...
<Warning>
Wallet pregeneration is currently only available for EVM-compatible blockchains.
</Warning>
...
The email address associated with the pregenerated wallet. The user must authenticate with this email to claim the wallet.
</ParamField>
...
Once a wallet has been pregenerated, the user can claim it by authenticating through your application using the same email address.
...
Magic automatically links the pregenerated wallet to the authenticated user. No additional steps required.
...
You can verify the user's wallet address matches the pregenerated one.

```javascript JavaScript
const metadata = await magic.user.getInfo();
console.log('User wallet:', metadata.publicAddress);
```
</Step>
</Steps>
...
async function fundPregeneratedWallet(toAddress, amount) {
...
await fundPregeneratedWallet(walletAddress, '0.1');
```
</Tab>
...
async function claimWallet(email) {
  ...
  const metadata = await magic.user.getInfo();
  console.log('Claimed wallet:', metadata.publicAddress);
  
  const balance = await getBalance(metadata.publicAddress);
