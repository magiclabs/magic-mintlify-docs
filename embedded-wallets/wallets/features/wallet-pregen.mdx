...
**The data key protecting the wallet's private key is split via Shamir's Secret Sharing so that no single party — Magic or the developer — holds a complete data key on their own; the end user's authentication is required to reconstruct it and use the wallet after claiming.**
