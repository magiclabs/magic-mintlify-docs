...
The Sign Confirmation feature allows you to protect users from front-end attacks when using Magic’s UI for on-chain signature requests such as [Transaction Signing](/embedded-wallets/wallets/features/transaction-signing) and [Personal Signatures](/embedded-wallets/wallets/features/personal-signatures) by popping them out to a safe, Magic-hosted browser window to confirm the action.
