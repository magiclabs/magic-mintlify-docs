...
<Tabs>
<Tab title="Magic Embedded Wallets">

Use [Magic Embedded Wallets](/embedded-wallets/introduction) to authenticate users directly in your frontend. The Magic SDK handles login and issues a DID token that you use as the JWT for UOA requests.
...
<Tab title="External Identity Provider">

Configure an external [identity provider](/server-wallets/express-api/identity-provider) (Auth0, Firebase Auth, NextAuth, or a custom OIDC provider) that issues JWTs for your users.

### Configure your provider

Register your JWT issuer, audience, and JWKS URL with UOA during app registration so it can validate your tokens.
