...
Additionally, you may use `deviceCheckUI=false` to customize the messaging shown to users when an unrecognized device is detected. You must use this option if you are also passing `showUI=false` to your email login method call, otherwise the user may not see a UI to continue registering their device. See [API Reference](/embedded-wallet/api/client-side-sdks/javascript#loginwithemailotp) for more info.

Device Registration also supports [Custom SMTP](/embedded-wallet/authentication/customization/custom-smtp), allowing you to customize the domain and sender that the device registration email is sent from.
