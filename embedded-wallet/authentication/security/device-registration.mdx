...
Additionally, you may use `deviceCheckUI=false` to customize the messaging shown to users when an unrecognized device is detected. You must use this option if you are also passing `showUI=false` to your email login method call, otherwise the user may not see a UI to continue registering their device. See [API Reference](/docs/embedded-wallet/api/client-side-sdks/javascript#loginwithemailotp) for more info.
