...
Once logged in, you will receive a DID token that can be used with our [Admin SDK](/docs/embedded-wallet/api/server-side-sdks/node#token-module) to verify the user's information and wallet address on the backend. You can also retrieve the user's wallet address and email using [`getInfo`](/docs/embedded-wallet/api/client-side-sdks/javascript#getinfo) for Web/React Native and `getMetadata` for iOS, Android, and Flutter.
...
- [Web API documentation](/docs/embedded-wallet/api/client-side-sdks/javascript#loginwithemailotp)
