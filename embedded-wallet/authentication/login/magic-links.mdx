...
Magic link SDK methods are available via the [Web client-side SDK](/embedded-wallet/api/client-side-sdks/javascript). These methods are deprecated on all mobile SDKs.

## Usage

The `loginWithMagicLink` function sends an authentication request to the user. The resolved value is a Decentralized ID token with a default 15-minute lifespan. **Refer to the **[**API documentation**](/embedded-wallet/api/overview) **for information on how to install and initialize Magic.**
...
* To assert a case-specific failed state, provide an [RPC error code](/embedded-wallet/api/client-side-sdks/javascript#rpcerror) with `test+fail_with_{RPC_ERROR_CODE}@magic.link`

<CodeGroup>
```javascript Success icon="square-js"
magic.auth.loginWithMagicLink({ email: 'test+success@magic.link' });
```

```javascript Fail icon="square-js"
magic.auth.loginWithMagicLink({ email: 'test+fail@magic.link' });
```

```javascript Fail With Error icon="square-js"
...
[How to add OTP email login on iOS](/embedded-wallet/api/client-side-sdks/ios#loginwithemailotp)

[How to add OTP email login on Android](/embedded-wallet/api/client-side-sdks/android#loginwithemailotp)

[How to add OTP email login on Flutter](/embedded-wallet/api/client-side-sdks/flutter#loginwithemailotp)

[How to add OTP email login in React Native](/embedded-wallet/api/client-side-sdks/javascript#loginwithemailotp)

## Resources

- [Error Codes](/embedded-wallet/api/client-side-sdks/javascript#error-codes)
