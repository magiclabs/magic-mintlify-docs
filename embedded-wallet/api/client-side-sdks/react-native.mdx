...
| `options.locale?`          | String           | Customize the language of Magic's modal, email and confirmation screen. See [Localization](/embedded-wallet/authentication/customization/localization) for more.                                                                                                                                                                                                                                                                                        |
...
- `backgroundColor?` (String): Used to render a custom background color. By default, the background will be white. If you have changed the background color as part of your [custom branding setup](/embedded-wallet/authentication/customization/login-ui#configuration), make sure to pass that color to `magic.Relayer`.
...
| --- | --- |
| Event Name | Definition |
| `id-token-created` | Returns an object containing a short lived, time bound ID token that can be used to verify the ownership of a user's wallet address on login.<br/><br/>[Read more](/embedded-wallet/authentication/customization/login-ui#usage-0) about this token and how to use it. |

### `showUI`

Displays the fully [navigable wallet](/embedded-wallet/wallets/customization/widget-ui) to the user that adheres to the toggled configurations on your developer dashboard’s Widget UI tab. ⁠ ⁠This is only supported for users who login with email or Google. User must be signed in for this method to return or else it will throw an error.
...
Access to [MFA](/embedded-wallet/authentication/features/mfa) and [account recovery](/embedded-wallet/authentication/features/account-recovery) require paid add-ons.
...
We have also added an optional `backgroundColor` prop to the `Relayer` to fix issues with `SafeAreaView` showing the background. By default, the background will be white. If you have changed the background color as part of your [custom branding setup](/embedded-wallet/authentication/customization/login-ui#configuration), make sure to pass your custom background color to `magic.Relayer`:
