...
description: "Create wallets and sign transactions using Express API v2 endpoints for wallet management and signing operations."
---

<Warning>
**Express API v1 will be retired on July 31, 2026.** After that date, `/v1/` endpoints will stop working. Wallets that have not been migrated to v2 by that date will be permanently inaccessible. See the [v1 to v2 migration guide](/server-wallets/express-api/v1-to-v2-migration).
</Warning>
...
The blockchain to create the wallet for. Currently supported: `ETH`
</ParamField>
...
<ParamField header="X-Magic-Chain" type="string" required post={["Header"]}>
The blockchain to sign for. Currently supported: `ETH`
</ParamField>
...
```bash cURL icon="square-terminal"
curl -X POST 'https://tee.express.magiclabs.com/v2/wallet/sign/eip7702' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer YOUR_JWT_TOKEN' \
  -H 'X-Magic-API-Key: YOUR_MAGIC_API_KEY' \
  -H 'X-OIDC-Provider-ID: YOUR_OIDC_PROVIDER_ID' \
...
```bash cURL icon="square-terminal"
curl -X POST 'https://tee.express.magiclabs.com/v2/wallet/sign/message' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer YOUR_JWT_TOKEN' \
  -H 'X-Magic-API-Key: YOUR_MAGIC_API_KEY' \
  -H 'X-OIDC-Provider-ID: YOUR_OIDC_PROVIDER_ID' \
