...
Unlike other signing endpoints, this endpoint computes the authorization hash internally. You only need to provide the delegation parameters.
...
- The wallet must have ETH (or the network's native token) to pay for gas when submitting the authorization on-chain
...
<ResponseField name="y_parity" type="integer" required>
