...
Send a `POST` request to `/v2/wallet/sign/eip7702` with the delegation parameters.
...
const signEip7702Authorization = async (
  ...
  const res = await fetch('https://tee.express.magiclabs.com/v2/wallet/sign/eip7702', {
    method: 'POST',
    headers: {
