...
const personalSign = async (data: string | Uint8Array) => {
  ...
  return await fetch('/v2/wallet/sign/message', { 
    method: 'POST', 
    headers: {
...
const signLegacyTransaction = async (tx: Transaction) => {
  ...
  const res = await fetch('/v2/wallet/sign/message', { 
    method: 'POST', 
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${YOUR_JWT_TOKEN}`,
...
```typescript Versioned Transaction
...
```typescript Partial Transaction Signing
import { Transaction, VersionedTransaction, PublicKey } from '@solana/web3.js';

const partialSignTransaction = async (serializedTx: Uint8Array, type: string, serializeConfig?: any) => {
  // type: 'legacy' for legacy transactions, anything else for versioned transactions
