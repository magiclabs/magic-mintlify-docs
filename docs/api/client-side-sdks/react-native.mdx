...
- `backgroundColor?` (String): Used to render a custom background color. By default, the background will be white. If you have changed the background color as part of your [custom branding setup](docs/authentication/customization/login-ui#configuration), make sure to pass that color to `magic.Relayer`.
...
<Note>Only available with Dedicated Wallet.</Note>
...
### [#Error Handling](#error-handling-0)
...
⁠
/_ Optionally, chain to the id token creation event if needed and configured _/
...
Access to [MFA](/docs/authentication/features/mfa) and [account recovery](docs/authentication/features/account-recovery) require paid add-ons.
...
The OpenID Module and it's members are accessible on the Magic SDK instance by the
`openid` property.
...
| -10010 | `InactiveRecipient`           | We were unable to deliver a Magic Link to the specified email address. The email address might be invalid.                                                        |
...
We hreaView`showing the background. By default, the background will be white. If you have changedave also added an optional`backgroundColor`prop to the`Relayer`to fix issues with`SafeA the background color as part of your [custom branding setup](docs/authentication/customization/login-ui#configuration), make sure to pass your custom background color to `magic.Relayer`:
