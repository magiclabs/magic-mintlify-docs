...
[#`requestUserInfoWithUI`](#requestuserinfowithui)
--------------------------------------------------
...
class MagicViewController: UIViewController {
    ...
    func logout() {
        ...
        // Assuming user is logged in
        let config = RequestUserInfoWithUIConfiguration(scope = WalletUserInfoScope(email = "user_email@example.com"))

        magic.wallet.requestUserInfoWithUI(configuration, response: { response in
            guard let result = response.result
                else { return print("Error:", response.error.debugDescription) }
            print("Result", result)
        })
