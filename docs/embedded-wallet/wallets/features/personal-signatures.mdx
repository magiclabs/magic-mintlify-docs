...
These methods allow dApps to verifiably prove the ownership of the user's account by getting a signature from their private key and using it to sign arbitrary and/or typed data. Additionally, it is possible to obtain a user's signature during login in a single step using [login with verification feature](docs/embedded-wallet/authentication/customization/login-ui#usage-0).
...
message: {
/_ - Anything you want. Just a JSON Blob that encodes the data you want to send - No required fields - This is DApp Specific - Be as explicit as possible when building out the message schema.
_/
contents: "Hello, Bob!",
...
* [Supported EVM RPC Methods](/docs/embedded-wallet/api/client-side-sdks/web#evm-rpc-methods)
