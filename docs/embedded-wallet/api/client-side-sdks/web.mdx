...
⁠If you're integrating with [OAuth](/docs/embedded-wallet/api/client-side-sdks/web#o-auth-module), [OpenID](#open-id-module) or [WebAuthn](/docs/embedded-wallet/api/client-side-sdks/web#web-authn-module), additional dependencies are needed. You'll find the installation instructions in their dedicated sections within this documentation.
...
| `options.testMode?`        | Boolean          | Enable [`testMode`](/docs/embedded-wallet/authentication/login/magic-links) to assert the desired behavior through the email address you provide to `loginWithMagicLink` without having to go through the auth flow.                                                                                                                                                                                                                                         |
...
try {
/_ Initiates update email flow to update a user's current email to a new one _/
...
﻿[Relevant Error Codes](/docs/embedded-wallet/api/client-side-sdks/web#error-codes)﻿
...
Access to [account recovery](docs/embedded-wallet/authentication/features/account-recovery) require paid add-ons.
...
Access to [account recovery](docs/embedded-wallet/authentication/features/account-recovery) require
paid add-ons.
...
<Note>
Access to [MFA](/docs/embedded-wallet/authentication/features/mfa) require paid add-ons. Available as of `magic-sdk v28.11.0`
</Note>
#### Arguments
...
Access to [MFA](/docs/embedded-wallet/authentication/features/mfa) require paid add-ons. Available
...
provider: '...' /_ 'google', 'facebook', 'apple', etc. _/,
redirectURI: 'https://your-app.com/your/oauth/callback',
scope: ['user:email'] /_ optional _/,
});
...
try {
/_ Assuming user is logged in _/
