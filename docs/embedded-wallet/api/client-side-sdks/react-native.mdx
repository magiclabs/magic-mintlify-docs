...
We hreaView`showing the background. By default, the background will be white. If you have changedave also added an optional`backgroundColor`prop to the`Relayer`to fix issues with`SafeA the background color as part of your [custom branding setup](docs/embedded-wallet/authentication/customization/login-ui#configuration), make sure to pass your custom background color to `magic.Relayer`:
