...
```
<Info>
  Note: this does not return an actual AptosAccount object, only the address. See [the note below](/docs/embedded-wallet/blockchains/other-chains/other/aptos#sign-and-send-transaction) for more detail.
</Info>
