...
Magic link SDK methods are available via the [Web client-side SDK](/docs/api/client-side-sdks/web). These methods are deprecated on all mobile SDKs.

## Usage


The `loginWithMagicLink` function sends an authentication request to the user. The resolved value is a Decentralized ID token with a default 15-minute lifespan. **Refer to the **[**API documentation**](/docs/api/overview) **for information on how to install and initialize Magic.**
...
const magic =  new  Magic('YOUR\_API\_KEY',  {
...
* To assert a case-specific failed state, provide an [RPC error code](/docs/api/client-side-sdks/web#rpcerror) with `test+fail_with_{RPC_ERROR_CODE}@magic.link`
...
[How to add OTP email login on iOS](/docs/api/client-side-sdks/ios#loginwithemailotp)

[How to add OTP email login on Android](/docs/api/client-side-sdks/android#loginwithemailotp)

[How to add OTP email login on Flutter](/docs/api/client-side-sdks/flutter#loginwithemailotp)

[How to add OTP email login on Unity](/docs/api/client-side-sdks/unity#loginwithemailotp)

[How to add OTP email login in React Native](/docs/api/client-side-sdks/web#loginwithemailotp)

## Resources


* [Error Codes](/docs/api/client-side-sdks/web#error-codes)
