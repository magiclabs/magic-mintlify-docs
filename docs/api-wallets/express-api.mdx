---
title: "TEE Express API"
description: "Complete reference for Magic's TEE Express API - a secure wallet management service that handles key management internally using trusted execution environment technology"
---
...
TEE Express requires multiple authentication headers for secure access:
...
Retrieve the wallet's public address for the given chain.
...
The blockchain to get the wallet address for. Available values: `ETH`, `SOL`
</ParamField>

<ResponseField name="public_address" type="string">
The wallet's public address for the specified chain.
</ResponseField>

### Create New Wallet

Create a new wallet for the given chain and return its public address.
...
<ParamField body="one_time_code" type="string">
Optional one-time code for additional security verification.
</ParamField>
...
2. **API Key/Secret Key**: Service-level authentication  
...
- Store JWT tokens securely and implement proper token refresh logic
- Never expose API keys or secret keys in client-side code
- Implement proper error handling to avoid information leakage

### Performance

- Cache wallet addresses to reduce API calls
- Implement retry logic with exponential backoff for transient failures
- Use appropriate timeouts for network requests

### Integration

- Test all signing operations thoroughly in a development environment
- Validate signature formats match your application's requirements
...
- **Documentation**: Visit our [comprehensive guides](https://magic.link/docs)
- **Community**: Join our [Discord server](https://discord.gg/magiclabs)
- **Support**: Contact our support team through the [Magic Dashboard](https://dashboard.magic.link)
